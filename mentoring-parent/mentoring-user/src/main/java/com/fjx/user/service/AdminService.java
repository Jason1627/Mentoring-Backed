package com.fjx.user.service;


import com.fjx.user.dao.AdminDao;
...
public class AdminService {
     ...
     * @Return: com.tensqure.user.pojo.Admin
     * @Author: Jason
     * @Date: 2020-04-28 15:43
     */
    public Admin finByLoginnameAndPassword(Admin admin ) {
        // 根据用户名查询用户信息
        Admin adminLogin  = adminDao.findByLoginname(admin.getLoginname());
        // 如果用户名不为空，且和数据库中的密码一致，则登陆成功
    ...
    public void add(Admin admin) {
        admin.setId(idWorker.nextId() + ""); // 主键
        // 密码加密
        ...
        return new Specification<Admin>() {
            ...
            public Predicate toPredicate(Root<Admin> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
                ...
                if (searchMap.get("loginname") != null && !"".equals(searchMap.get("loginname"))) {
                    predicateList.add(cb.like(root.get("loginname").as(String.class), "%" + (String) searchMap.get(
                    		"loginname") + "%"));
                }
                // 密码
